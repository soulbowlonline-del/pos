...
reset() {
  ...
  docker exec pos-mysql-8 sh -c 'mysql -uroot -p$MYSQL_ROOT_PASSWORD $MYSQL_DATABASE -e "
    ...
    DELETE FROM tbl_whatsapp_logs WHERE number IN (\"9995550001\",\"9990001234\") OR number=\"\" OR number IS NULL;"' >/dev/null 2>&1
}
...
state() {
  docker exec -i pos-mysql-8 sh -c 'mysql -uroot -p$MYSQL_ROOT_PASSWORD -N $MYSQL_DATABASE -e "
    ...
    SELECT CONCAT(\"log:\",number,\"|\",status,\"|\",template_name) FROM tbl_whatsapp_logs WHERE number IN (\"9995550001\",\"9990001234\") OR number=\"\" OR number IS NULL ORDER BY id;"' 2>/dev/null | norm
}

# Some cases need the fixture customer altered before the call (a punctuated
# phone number, say). The SQL runs after reset, so it survives it.
setup_extra() {
  [ -z "$1" ] && return
  docker exec pos-mysql-8 sh -c "mysql -uroot -p\$MYSQL_ROOT_PASSWORD \$MYSQL_DATABASE -e '$1'" >/dev/null 2>&1
}

run_case() {
  local name="$1" a1="$2" a2="$3" data="$4" q="$5" extra="$6"
  reset; setup_extra "$extra"
  local r1; r1=$(curl -sS --max-time 120 -X POST -d "$data" "$BASE/api/customer/$a1$q" | norm)
  local o1; o1=$(outbound); local s1; s1=$(state)
  reset; setup_extra "$extra"
  local r2; r2=$(curl -sS --max-time 120 -X POST -d "$data" "$BASE/v2/api/customer/$a2$q" | norm)
...
echo "=== customer/add + sentwhatappotp + uploadwhatapporder differential (outbound stubbed) ==="
...
run_case "uploadwhatapporder valid"   uploadwhatapporder uploadwhatapporder "" "?id=9990001"
run_case "uploadwhatapporder unknown" uploadwhatapporder uploadwhatapporder "" "?id=98765432"
run_case "uploadwhatapporder punctuated phone" uploadwhatapporder uploadwhatapporder "" "?id=9990001" \
         'UPDATE tbl_customer SET contact_no="+91 999-000-1234" WHERE id=9990001'
run_case "uploadwhatapporder blank phone"      uploadwhatapporder uploadwhatapporder "" "?id=9990001" \
         'UPDATE tbl_customer SET contact_no="" WHERE id=9990001'
echo
