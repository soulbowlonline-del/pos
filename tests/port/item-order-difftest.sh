#!/bin/bash
# Differential test for item/order - the till's checkout.
#
...
run_case() {
  ...
  reset; local r1; r1=$(curl -sS --max-time 180 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=$items" -d "$body" "$BASE/api/item/order" | norm); local w1; w1=$(written)
  reset; local r2; r2=$(curl -sS --max-time 180 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=$items" -d "$body" "$BASE/v2/api/item/order" | norm); local w2; w2=$(written)
  if [ "$r1" = "$r2" ] && [ "$w1" = "$w2" ]; then
...
run_error_case() {
  local name="$1" body="$2" items="$3"
  reset; local c1; c1=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 180 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=$items" -d "$body" "$BASE/api/item/order"); local w1; w1=$(written)
  reset; local c2; c2=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 180 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=$items" -d "$body" "$BASE/v2/api/item/order"); local w2; w2=$(written)
  if [ "$c1" = "$c2" ] && [ "$w1" = "$w2" ]; then
...
echo "=== item/order differential  (modes $MODE_PAY/$MODE_DEL) ==="
...
run_case "two lines of the same item"         "$B&status_id=1" "[$LINE,$LINE]"
...
reset; a=$(curl -sS --max-time 60 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=[$LINE]" -d "status_id=1&outlet_id=$OUTLET" "$BASE/api/item/order")
reset; b=$(curl -sS --max-time 60 -X POST -H "userlogin: $LOGIN" --data-urlencode "item_details=[$LINE]" -d "status_id=1&outlet_id=$OUTLET" "$BASE/v2/api/item/order")
