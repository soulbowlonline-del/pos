...
# fixtures it needs.
cd /root/pos
SUITES="rows_difftest deletes_difftest hooks_difftest classrefs_difftest staticcalls_difftest login_difftest difftest emp_difftest cust_difftest order_difftest tally_difftest item_difftest
...
for s in $SUITES; do
  ...
  out=$(/root/pos/$s.sh 2>&1)
  r=$(echo "$out" | grep -oE 'passed: [0-9]+ +mismatched: [0-9]+')
  # A suite may also report cases where nothing was compared - an
  # unordered paginated listing, a page both stacks render empty. They
  # are neither passes nor mismatches, and dropping them here would hide
  # a gap in coverage behind an all-green summary.
  nc=$(echo "$out" | grep -oE 'nothing-compared: [0-9]+' | tail -1)
