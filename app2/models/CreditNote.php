...
class CreditNote extends ActiveRecord
{
    ...
    {
        return $n == 1 ? 'CreditNote' : 'CreditNotes';
    }
    ...
    {
        $value = $this->hasAttribute('credit_number') ? $this->credit_number : null;
    ...
    public function rules()
    {
        return [
            ...
            [['amt', 'amt_used'], 'number'],
            [['credit_number'], 'string', 'max' => 255],
            [['create_time', 'update_time'], 'safe'],
            [['amt', 'amt_used', 'type_id', 'status', 'create_time', 'update_time'], 'default', 'value' => null],
            [['id', 'credit_number', 'amt', 'amt_used', 'type_id', 'status', 'create_time', 'update_time'], 'safe', 'on' => 'search'],
        ];
    }
    ...
    public function search($params = [])
    {
		...
		Criteria::compare($query, 'credit_number', $this->credit_number, true);
		Criteria::compare($query, 'amt', $this->amt);
		Criteria::compare($query, 'amt_used', $this->amt_used);
