...
class ScannedItems extends ActiveRecord
{
    ...
    {
        return $n == 1 ? 'ScannedItem' : 'ScannedItems';
    }
    ...
    {
        $value = $this->hasAttribute('computer_name') ? $this->computer_name : null;
    ...
    public function getScannedItemsColumns($selectcolumns = []) {
    ...
    public function getGetItemDetail()
    {
        return $this->hasOne(ItemDetail::class, ['id' => 'item_id']);
    }

    /**
     * Yii 1's scannedItemsearch(): a listing of its own, converted as written.
     */
    public function scannedItemsearch()
    {
		...
		if((Yii::$app->session['scanned_start_date'] != '') && (Yii::$app->session['scanned_end_date'] != '')){
			$query->andWhere(['between', 'DATE(created_at)', Yii::$app->session['scanned_start_date'], Yii::$app->session['scanned_end_date']]);
		} 
		...
		Criteria::compare($query, 'user_email', $this->user_email);
		Criteria::compare($query, 'item_id', $this->item_id);
		Criteria::compare($query, 'bar_code', $this->bar_code);
		Criteria::compare($query, 'is_coupon', $this->is_coupon);
		Criteria::compare($query, 'qty', $this->qty); 
	 	Criteria::compare($query, 'sale_rate', $this->sale_rate);
		Criteria::compare($query, 'base_price', $this->base_price);
    ...
    /**
     * Yii 1's scannedItemsSearchExport(): a listing of its own, converted as written.
     */
