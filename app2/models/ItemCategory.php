...
class ItemCategory extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['title'], 'unique'],
            [['create_time', 'update_time', 'start_date', 'end_date', 'columns'], 'safe'],
            ...
            if ($count > 1) {
                ...
                try {
                    for ($i = 1; $i < $count; $i++) {
                        ...
                        if (isset($arrays['Category']) || isset($arrays['﻿"Category"']) || isset($arrays['���"Category"'])) {

                            if (isset($arrays['Category'])) {
                                $itemcategory->title = $itemcat_values[$arrays['Category']];

                            } else if(isset($arrays['﻿"Category"'])) {
                                $itemcategory->title = $itemcat_values[$arrays['﻿"Category"']];
                            }else{
                                $itemcategory->title = $itemcat_values[$arrays['���"Category"']];
                            }
                        ...
                        if (isset($arrays['Parent Category'])) {
                            $query = ItemCategory::find();
            $query->orderBy(['id' => SORT_DESC]);
                            Criteria::compare($query, 'title', $itemcat_values[$arrays['Parent Category']]);
                            $category = $query->one();
                            if($category){
    ...
    /**
     * Yii 1's subcatsearch(): a listing of its own, converted as written.
     */
