...
class ItemReturn extends ActiveRecord
{
    ...
    {
        return $n == 1 ? 'ItemReturn' : 'ItemReturns';
    }
    ...
    public function rules()
    {
        return [
            ...
            [['vendor_id', 'outlet_id', 'status', 'type_id', 'credit_note_id', 'create_user_id', 'updated_by'], 'integer'],
            [['discount_amt', 'other_charge'], 'number'],
            [['credit_note_no', 'bill_no', 'invoice_no', 'grn_no', 'credit_note_date', 'grn_save_date'], 'safe'],
            [['total_amt'], 'string', 'max' => 10],
            [['total_amt', 'status', 'type_id', 'updated_by'], 'default', 'value' => null],
            [['credit_note_no', 'bill_no', 'invoice_no', 'grn_no', 'id', 'discount_amt', 'other_charge', 'total_amt', 'vendor_id', 'outlet_id', 'status', 'type_id', 'create_time', 'credit_note_id', 'create_user_id', 'updated_by'], 'safe', 'on' => 'search'],
        ];
    }
    ...
    public function search($params = [])
    {
        ...
        foreach ([['id', 'id'], ['discount_amt', 'discount_amt'], ['other_charge', 'other_charge'], ['vendor_id', 'vendor_id'], ['outlet_id', 'outlet_id'], ['status', 'status'], ['type_id', 'type_id'], ['credit_note_id', 'credit_note_id'], ['create_user_id', 'create_user_id'], ['updated_by', 'updated_by']] as [$col, $attr]) {
            Criteria::compare($query, $col, $this->$attr);
        }
        foreach ([['total_amt', 'total_amt'], ['create_time', 'create_time']] as [$col, $attr]) {
    ...
    {
        return $this->hasOne(CreditNote::class, ['id' => 'credit_note_id']);
    }

    public function getItemReturnItems()
    {
        return $this->hasMany(ItemReturnItem::class, ['return_id' => 'id']);
    }
