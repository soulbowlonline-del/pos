...
class OnlineOrderItem extends ActiveRecord
{
    ...
    {
        return $n == 1 ? 'OnlineOrderItem' : 'OnlineOrderItems';
    }
    ...
    /**
     * GxActiveRecord::getRelationLabel(): a relation or foreign key
     * resolves to the *related* model's label, and anything else falls
     * through to the attribute label. attributeLabels() deliberately
     * does not do this - getAttributeLabel() answers a null Yii 1 label
     * with the generated one, and a detail view naming a foreign key
     * shows "Discount Id" where a view asking this shows "Discount".
     */
    ...
    public function rules()
    {
        return [
            [['name'], 'required'],
            [['type_id', 'create_user_id', 'updated_by'], 'integer'],
            [['price', 'total'], 'number'],
            [['name', 'status'], 'string', 'max' => 255],
            [['image_url', 'create_time', 'update_time', 'barcode', 'product_code'], 'safe'],
            [['qty', 'price', 'total', 'image_url', 'type_id', 'status', 'create_time', 'update_time', 'create_user_id', 'updated_by'], 'default', 'value' => null],
            [['id', 'name', 'qty', 'price', 'total', 'image_url', 'type_id', 'status', 'create_time', 'update_time', 'create_user_id', 'updated_by'], 'safe', 'on' => 'search'],
        ];
    }
    ...
    public function search($params = [])
    {
        ...
        foreach ([['id', 'id'], ['qty', 'qty'], ['price', 'price'], ['total', 'total'], ['type_id', 'type_id'], ['create_user_id', 'create_user_id'], ['updated_by', 'updated_by']] as [$col, $attr]) {
            Criteria::compare($query, $col, $this->$attr);
        }
        foreach ([['name', 'name'], ['image_url', 'image_url'], ['status', 'status'], ['create_time', 'create_time'], ['update_time', 'update_time']] as [$col, $attr]) {
