...
echo GridView::widget([
		...
		/* 	array(
			...
			array(
				...
				[
						'header' => 'Tax',
						'value' => function ($data) { return Html::dropDownList("PurchaseBillDetail[tax_id]",$data->tax_id,$data->getAllTaxOptions($data->tax_id),["id"=>"select_tax$data->id","class"=>"select_tax"]); },
						'format' => 'raw',
						'htmlOptions' => [
...
function gridcalculation(id,val=1){
	...
	if(price != '' &&  discount != ''){
		...
		var Discount_cal= qty * price;
		var totalPrice= Discount_cal + parseFloat(CGST_amt) + parseFloat(SGST_amt) + parseFloat(CESS_amt) + parseFloat(IGST_amt);
		
		var discount_amt = totalPrice * discount/100;
		...
		var total_Igst = parseFloat(price_igst) + parseFloat(price_cess);
		 var gst = parseFloat(total_Igst);
		<?php }?>
		 ...
		 var calculated_gross = 0;
		 <?php if($model->getGSTTrue($poid) == true){?>
		var total_amount =  parseFloat(taxable_amount) + parseFloat(CGST_amt) +  parseFloat(SGST_amt) + parseFloat(CESS_amt) - parseFloat(calculate_discount);
       <?php }else{?>
       var total_amount =  parseFloat(taxable_amount) + parseFloat(IGST_amt) + parseFloat(CESS_amt) - parseFloat(calculate_discount);
       <?php }?>
...
function calculation(val = 1){
	...
	<?php if($model->getGSTTrue($poid) == true){?>
			...
			var price_sgst = parseFloat(price) * SGST_per/100;
			var price_cess = parseFloat(price) * CESS_per/100;
			 var gst = parseFloat(price_cgst)+parseFloat(price_sgst) + parseFloat(price_cess) - parseFloat(calculate_discount);
			<?php }else{?>
			var price_cess = parseFloat(price) * CESS_per/100;
		var price_igst = parseFloat(price) * IGST_per/100;
		var Total__Igst = parseFloat(price_igst) + parseFloat(price_cess) - parseFloat(calculate_discount);
		 var gst = parseFloat(Total__Igst);
			<?php }?>
