...
if ($poid) {
	$mrs = B2bPurchaseBill::find()->where([
			'id' => $poid 
	])->one();
	if ($mrs) {
...
<section class="content-header">
	<h1> <?php echo 'Manage' ;?> B2B PurchaseBillDetails</h1>
...
$form = ActiveForm::begin([
		...
		'action' => Ui::to( 'b2bpurchaseBillDetail/index' ),
		'htmlOptions' => [
...
?>
	<div class="add-item">
                  ...
                  <?php echo $form->textField($model,'bar_code',['class'=>'form-control','id'=>'PurchaseBillDetail_bar_code']); ?>
										</div>
											<div class="col-md-2 col-xs-12 padding2px">
   ...
   <?php echo $form->error($model,'item_id');?>
                 ...
                 <?php echo $form->textField($model,'approved_qty',['class'=>'form-control']); ?>
                  ...
                  <?php echo $form->dropDownList($model, 'is_free',$model->getFreeItemOptions(),['class'=>'form-control']); ?>
...
echo $form->datepickerRow ( $model, 'bill_date', [
		'hint' => 'Click inside! to select a date.',
		'prepend' => '<i class="icon-calendar"></i>',
		'options' => [
				'format' => 'yyyy-mm-dd',
...
echo GridView::widget([
		...
		'columns' => [
				...
				[
						...
						'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'hsn_code', ["id"=>"hsn_code_input$data->id","value"=>$data->gethsncode(),"class"=>"hsn_code_input"]); },
						'format' => 'raw',
						'htmlOptions' => [
		...
		/* 	array(
			...
			array(
				...
				/* array (
						'header' => 'Free',
						'value' => function ($data, $key, $index) { return $data->getFreeItemOptions($data->is_free); },
						'filter' => false,
						'htmlOptions' => array (
								'id' => 'free_field'
						)
				...
				[
						...
						'header' => 'Free Val',
						'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'is_free', ["id"=>"free_val$data->id","class"=>"free_val_qty","value"=>$data->getFreeItemOptions($data->is_free),"readOnly"=>"readOnly"]); },
						'format' => 'raw',
						'htmlOptions' => [
				...
				[
						'header' => 'Tax',
						'value' => function ($data, $key, $index) { return Html::dropDownList("B2bPurchaseBillDetail[tax_id]",$data->tax_id,$data->getAllTaxOptions($data->tax_id),["id"=>"select_tax$data->id","class"=>"select_tax"]); },
						'format' => 'raw',
						'htmlOptions' => [
				...
				[
					...
					'value' => function ($data, $key, $index) { return Html::dropDownList("B2bPurchaseBillDetail[tax_id]",$data->tax_id,$data->getAllTaxOptions($data->tax_id),["id"=>"select_tax_tcstax$data->id","class"=>"select_tax_tcstax"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'CGST (%age)',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'cgst_per',["id"=>"CGST_per_input_tcstax$data->id","class"=>"CGST_per_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'SGST (%age)',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'sgst_per',["id"=>"SGST_per_input_tcstax$data->id","class"=>"SGST_per_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					// 'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'CESS (%age)',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'cess_per',["id"=>"CESS_per_input_tcstax$data->id","class"=>"CESS_per_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'CGST Amt',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'cgst_amt',["id"=>"CGST_amt_input_tcstax$data->id","class"=>"CGST_amount_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'SGST Amt',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'sgst_amt',["id"=>"SGST_amt_input_tcstax$data->id","class"=>"SGST_amount_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					// 'visible' => $model->getGSTTrue ( $poid ) == true,
					'header' => 'CESS Amt',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'cess_amt',["id"=>"CESS_amt_input_tcstax$data->id","class"=>"CESS_amount_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					// 'visible' => $model->getGSTTrue ( $poid ) == false,
					'header' => 'IGST (%age)',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'igst_per',["id"=>"IGST_per_input_tcstax$data->id","class"=>"IGST_per_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
			...
			[
					// 'visible' => $model->getGSTTrue ( $poid ) == false,
					'header' => 'IGST Amt',
					'value' => function ($data, $key, $index) { return Html::activeTextInput($data,'igst_amt',["id"=>"IGST_amt_input_tcstax$data->id","class"=>"IGST_amount_input_tcstax","ReadOnly"=>"ReadOnly"]); },
					'format' => 'raw',
					'htmlOptions' => [
...
$('#Bill_vendor_id').change(function(){
	...
	var url = "<?php echo Ui::to('b2bpurchaseBillDetail/index'); ?>/vid/"+vendor;
	location.href = url; 
});
...
$(document).ready(function(){
	...
	$("#update").click(function (event) {
		...
		var link = '<?php echo Ui::to('b2bpurchaseBillDetail/list'); ?>';
	...
	$("td.hsn_code_input input:text").each(function(key,value){
		...
		$.ajax({
		       url: '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxupdate'); ?>/id/'+outlet,
		       type: 'post',
	...
	$("#approve").click(function (event) {
		...
		if(bill_no != ''){
			$.ajax({
				url: '<?php echo Ui::to('B2bpurchaseBillDetail/ajaxBillNo'); ?>/id/'+poid,
				type: 'post',
				...
				success: function (data) {
					...
					$.ajax({
						url: '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxupdate'); ?>/id/'+poid,
						type: 'post',
...
if ($poid) {
	$getPurchaseBill = B2bPurchaseBill::findOne( $poid );
...
$('#PurchaseBillDetail_purchase_bill_id').change(function(){
	...
	var poid = $('#PurchaseBillDetail_purchase_bill_id').val();
	var url = '<?php echo Ui::to('b2bpurchaseBillDetail/index')?>/id/'+vendor_id+'/poid/'+poid;
...
$('#credit_note').change(function(){
	...
	var bill_amount  = $('#before_net_bill_amount').val();
	   jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('purchaseBillDetail/applyCreditNote') ?>/id/'+poid,
...
function checkBarcodes(){
	 var item_id = $('#item_hsn_code').val();
	    jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxItems') ?>',
...
//	 var item_detail_id = $('#PurchaseBillDetail_item_detail_id').val();
 jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxPBillTax') ?>',
...
/*
								       ...
								       * jQuery.ajax({
								       * 'type': 'POST',
								       * 'url': '<?php echo Ui::to('purchaseBillDetail/ajaxPBillTax') ?>',
								       * 'data': {'item_id': item_id,'item_detail_id':item_detail_id,'vendor_id':vendor_id},
								       * dataType: 'json',
								       * 'success': function (data) {
								       * $('#item_tax_data').html('');
...
function checkPONo(vendor_id){
	...
	<?php }?>
	   jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxPONo') ?>',
...
function checkPendingBillDate(bill_id){
	   ...
	   jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/getpendingbilldate') ?>',
...
function checkBillNo(vendor_id){
	var vendor_id = vendor_id;
	   jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/getpendingbill') ?>',
	       'data': {'vendor_id': vendor_id},
	       'success': function (data) {
...
function checkTaxVal(tax_id,id){
	  jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxTax') ?>',
	       'data': {'tax_id': tax_id},
	       dataType: 'json',
	       'success': function (data) {
...
function checkTaxTable(tax_id,id){
  ...
  //  console.log('igst'+igst);
	...
	jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxTaxTable') ?>',
...
if(change == 0){
 ...
 jQuery.ajax({
     'type': 'POST',
     'url': '<?php echo Ui::to('b2bpurchaseBillDetail/ajaxCreate') ?>/id/'+outlet,
