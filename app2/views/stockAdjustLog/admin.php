...
<?php $gridWidget = GridView::widget([
	...
	'columns' => [
		...
		'date',
			...
			[
				...
				// 'value' => function ($data) { return $data->getUserNameById(); },createUser
				'value' => function ($data) { return isset($data->createUser)?$data->createUser:""; },
				'filter'=>Gx::listData(User::findAll([], ['order'=>'full_name ASC'])),
				],
		[
		...
		'current_stock',
			...
			[
					'header'=>'amount',
					'value' => function ($data) { return $data->getAdjustedAmount(); },
					//'filter'=>Gx::listData(Item::class),
			],
		/*
		'actual_stock',
...
<?php $this->renderExportGridButton($gridWidget,'Export Grid Results',['class'=>'btn btn-info pull-right']);?>
</div>
</div>
</div>
</div>
