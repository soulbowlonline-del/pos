...
/**
 * Yii 2 port of protected/controllers/ItemVendorController.php.
 *
 ...
 */
class ItemVendorController extends BaseUiController {
	...
	public function actionDelete($id) 
	{
		...
		if (Yii::$app->request->isPost) {
			...
			if (!Yii::$app->request->isAjax)
				return $this->redirect(['item/vendor','id'=>$item_id]);
		} else
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => ItemVendor::find(),
            ...
            'sort' => ['defaultOrder' => ItemVendor::defaultOrder() ?: []],
	...
	{	
		...
		if ( $model == null ) $model = new ItemVendor();
		
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('itemVendor/view', ['id'=>$model->id]),'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('itemVendor/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'List', 'url' => Ui::to('itemVendor/index'),'icon'=>'icon-th-list icon-white'];	
				}
				break;				
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('itemVendor/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('itemVendor/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
			...
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('itemVendor/create'),'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('itemVendor/update', ['id' => $model->id]), 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
