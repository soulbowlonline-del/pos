...
/**
 * Yii 2 port of protected/controllers/ShiftController.php.
 *
 ...
 */
class ShiftController extends BaseUiController {
	...
	public function actionView($id) 
	{
		...
		if( !($model->checkPermission ('shift/view')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = new Shift;
		if( !($model->checkPermission ('shift/create')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = $this->loadModel($id);
		if( !($model->checkPermission ('shift/update')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => Shift::find(),
            ...
            'sort' => ['defaultOrder' => Shift::defaultOrder() ?: []],
	...
	{	
		...
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('shift/view', ['id'=>$model->id]),'visible'=> $model->checkPermission ("shift/view")=="true",'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('shift/admin'),'visible'=> $model->checkPermission ("shift/admin")=="true",'icon'=>'icon-wrench icon-white'];							
			...
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('shift/admin'),'visible'=> $model->checkPermission ("shift/admin")=="true",'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('shift/create'),'visible'=> $model->checkPermission ("shift/create")=="true",'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
			...
			case 'view':
				...
				//	'confirm'=>'Are you sure you want to delete this item?'),'icon'=>'icon-remove icon-white');
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('shift/create'),'visible'=> $model->checkPermission ("shift/create")=="true",'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('shift/update', ['id' => $model->id]),'visible'=> $model->checkPermission ("shift/update")=="true", 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
