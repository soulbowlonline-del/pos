...
/**
 * Yii 2 port of protected/controllers/OrganizationController.php.
 *
 ...
 */
class OrganizationController extends BaseUiController {
	...
	public function actionView($id) 
	{
		...
		if( !($model->checkPermission ('organization/view')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = new Organization;
		if( !($model->checkPermission ('organization/create')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = $this->loadModel($id);
		if( !($model->checkPermission ('organization/update')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => Organization::find(),
            ...
            'sort' => ['defaultOrder' => Organization::defaultOrder() ?: []],
	...
	{	
		...
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('organization/view', ['id'=>$model->id]),'visible'=> $model->checkPermission ("organization/view")=="true",'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('organization/admin'),'visible'=> $model->checkPermission ("organization/admin")=="true",'icon'=>'icon-wrench icon-white'];							
					//$this->menu[] = array('label'=>'List', 'url'=>array('index'),'icon'=>'icon-th-list icon-white');	
				}
				break;				
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('organization/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('organization/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
				...
				//	$this->menu[] = array('label'=>'List', 'url'=>array('index'),'icon'=>'icon-th-list icon-white');
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('organization/create'),'visible'=> $model->checkPermission ("organization/create")=="true",'icon'=>'icon-plus icon-white'];
				}
				break;				
			default:
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('organization/create'),'visible'=> $model->checkPermission ("organization/create")=="true",'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('organization/update', ['id' => $model->id]),'visible'=> $model->checkPermission ("organization/update")=="true", 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
