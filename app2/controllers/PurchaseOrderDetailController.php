...
/**
 * Yii 2 port of protected/controllers/PurchaseOrderDetailController.php.
 *
 ...
 */
class PurchaseOrderDetailController extends BaseUiController {
	...
	public function actionSendEmail($id){
		...
		$mPDF1->Output("whatsapp/$id.pdf",'F');
		 ...
		 $vendor = Vendor::findOne($purchaseorder->vendor_id);
		if($vendor){
			$vendoruser = User::findOne($vendor->create_user_id);
			if($vendoruser){
		...
		$view = $this->renderPartial ( '/mail/purchase_order_approved', [
	...
	{
		$existpo = $this->loadModel($id, PurchaseOrder::class);
		...
		if (isset($_POST['PurchaseOrderDetail'])) {
			...
			$model->load($_POST, 'PurchaseOrderDetail');
				...
				if ($model->save()) {
					if($model->approved_qty != '' && $model->approved_qty != '0'){
						...
						/* if($billmodel->save()){
							...
							$billdetailmodel = PurchaseBillDetail::findOne(array('purchase_bill_id'=>$billmodel->id,'outlet_id'=>$model->outlet_id));
			...
			if ($model->save()) {
				if($model->approved_qty != '' && $model->approved_qty != '0'){
					...
					if($billmodel->save()){
						...
						$billdetailmodel = PurchaseBillDetail::findOne(['purchase_bill_id'=>$billmodel->id,'outlet_id'=>$model->outlet_id]);
						if($billdetailmodel == null){
							$billdetailmodel = new PurchaseBillDetail();
						}
				...
				return $this->redirect(['admin', 'id' => $existpo->vendor_id,'poid'=>$id]);
			}
		}
		$this->updateMenuItems($model);
	...
	{	
		...
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('purchaseOrderDetail/view', ['id'=>$model->id]),'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('purchaseOrderDetail/admin'),'icon'=>'icon-wrench icon-white'];							
			...
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('purchaseOrderDetail/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('purchaseOrderDetail/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
				...
				//	$this->menu[] = array('label'=>'List', 'url'=>array('index'),'icon'=>'icon-th-list icon-white');
					$this->menu[] = ['label'=>'Add Item', 'url' => Ui::to('purchaseOrderDetail/create', ['id'=>$poid]),'icon'=>'icon-plus icon-white'];
				}
				break;				
			default:
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('purchaseOrderDetail/create'),'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('purchaseOrderDetail/update', ['id' => $model->id]), 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
		...
		if(count($qtys)>0)
		{
			...
			if($purchasebill->save()){
				...
				foreach($qtys as $key=>$qty)
				{
					...
					$purchaseordermodel = $this->loadModel($model->purchase_order_id, PurchaseOrder::class);
					...
					if($model->save()){
						if($qty != '' && $qty != '0'){
							$purchasedetailmodel = PurchaseBillDetail::findOne(['purchase_bill_id'=>$purchasebill->id,'outlet_id'=>$purchasebill->outlet_id,'item_id'=>$model->item_id,'item_detail_id'=>$model->item_detail_id]);
			...
			$mrn = Mrn::findOne($purchaseorder->mrn_id);
