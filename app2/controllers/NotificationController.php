...
/**
 * Yii 2 port of protected/controllers/NotificationController.php.
 *
 ...
 */
class NotificationController extends BaseUiController {
	...
	public function actionCreate() 
	{
		...
		if (Yii::$app->request->post('Notification') !== null) {
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => Notification::find(),
            ...
            'sort' => ['defaultOrder' => Notification::defaultOrder() ?: []],
	...
	{
		$model = new Notification(['scenario' => 'search']);
		$this->updateMenuItems($model);
	
		if (Yii::$app->request->get('Notification') !== null)
		{
	...
	{	
		...
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('notification/view', ['id'=>$model->id]),'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('notification/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'List', 'url' => Ui::to('notification/index'),'icon'=>'icon-th-list icon-white'];	
				}
				break;				
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('notification/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('notification/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
			...
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('notification/create'),'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('notification/update', ['id' => $model->id]), 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
