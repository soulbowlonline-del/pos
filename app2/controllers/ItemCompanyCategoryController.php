...
/**
 * Yii 2 port of protected/controllers/ItemCompanyCategoryController.php.
 *
 ...
 */
class ItemCompanyCategoryController extends BaseUiController {
		...
		if (Yii::$app->request->post('ItemCompanyCategory') !== null) {
			...
			if ($model->save()) {
				...
				if($user->role_id == $role->id ){
					return $this->redirect(['itemCompany/view', 'id' => $id]);
				}else{
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => ItemCompanyCategory::find(),
            ...
            'sort' => ['defaultOrder' => ItemCompanyCategory::defaultOrder() ?: []],
	...
	{	
		...
		if ( $model == null ) $model = new ItemCompanyCategory();
		
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('itemCompanyCategory/view', ['id'=>$model->id]),'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('itemCompanyCategory/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'List', 'url' => Ui::to('itemCompanyCategory/index'),'icon'=>'icon-th-list icon-white'];	
				}
				break;				
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('itemCompanyCategory/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('itemCompanyCategory/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
			...
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('itemCompanyCategory/create'),'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('itemCompanyCategory/update', ['id' => $model->id]), 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
