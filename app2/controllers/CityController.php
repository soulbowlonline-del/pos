...
/**
 * Yii 2 port of protected/controllers/CityController.php.
 *
 ...
 */
class CityController extends BaseUiController {
	...
	public function actionView($id) 
	{
		...
		if( !($model->checkPermission ('city/view')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = new City;
		if( !($model->checkPermission ('city/create')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	{
		$model = $this->loadModel($id);
		if( !($model->checkPermission ('city/update')))	throw new ForbiddenHttpException('You are not allowed to access this page.');
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new ActiveDataProvider(['query' => City::find(),
            ...
            'sort' => ['defaultOrder' => City::defaultOrder() ?: []],
		...
		if (isset($_GET['City']))
			...
			$this->exportCSV( $model->search (), [
					...
					[
							'label' => 'State',
							'value' => function ($data) {
							return State::getStateName($data->state_id);
							}],
						
					[
							'label'=>'Country',
							'value'=> function ($data) {
							return State::getStateCountryName($data->state_id);
							}],
	...
	{	
		...
		if ( $model == null ) $model = new City();
		
		switch( $this->action->id)
		{
			case 'update':	
				{
					$this->menu[] = ['label'=>'View' , 'url' => Ui::to('city/view', ['id'=>$model->id]),'visible'=> $model->checkPermission ("city/view")=="true",'icon'=>'icon-plus icon-white'];
				}
			case 'create':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('city/admin'),'visible'=> $model->checkPermission ("city/admin")=="true",'icon'=>'icon-wrench icon-white'];							
					//$this->menu[] = array('label'=>'List', 'url'=>array('index'),'icon'=>'icon-th-list icon-white');	
				}
				break;				
			case 'index':
				{
					$this->menu[] = ['label'=>'Manage', 'url' => Ui::to('city/admin'),'icon'=>'icon-wrench icon-white'];							
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('city/create'),'icon'=>'icon-plus icon-white'];
				}
				break;
			case 'admin':
				{
				//	$this->menu[] = array('label'=>'List', 'url'=>array('index'),'icon'=>'icon-th-list icon-white');
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('city/create'),'visible'=> $model->checkPermission ("city/create")=="true",'icon'=>'icon-plus icon-white'];
				}
				break;				
			default:
			case 'view':
				{
					...
					$this->menu[] = ['label'=>'Create', 'url' => Ui::to('city/create'),'visible'=> $model->checkPermission ("city/create")=="true",'icon'=>'icon-plus icon-white'];
					$this->menu[] = ['label'=>'Update', 'url' => Ui::to('city/update', ['id' => $model->id]),'visible'=> $model->checkPermission ("city/update")=="true", 'icon'=>'icon-edit icon-white'];				
				}
				break;
		}
