...
class ItemCompanyCategoryController extends GxController {
	...
	{
		$model = $this->loadModel($id, 'ItemCompanyCategory');
	...
	{
		if($id != null)
		$company = $this->loadModel($id, 'ItemCompany');
		...
		if (isset($_POST['ItemCompanyCategory'])) {
			$model->setAttributes($_POST['ItemCompanyCategory']);
			if($id != null)
			$model->company_id = $id;
			if ($model->save()) {
				...
				if($user->role_id == $role->id ){
					$this->redirect(array('itemCompany/view', 'id' => $id));
				}else{
	...
	public function actionUpdate($id) 
	{
		...
		if (isset($_POST['ItemCompanyCategory'])) {
			...
			if ($model->save()) {
				$this->redirect(array('itemCompany/view', 'id' => $model->company_id));
			}
		}
		$this->updateMenuItems($model);
	...
	public function actionDelete($id) 
	{
		...
		if (Yii::app()->getRequest()->getIsPostRequest()) {
			$this->loadModel($id, 'ItemCompanyCategory')->delete();
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new CActiveDataProvider('ItemCompanyCategory');
	...
	{
		$model = new ItemCompanyCategory ('search');
		$model->unsetAttributes();
		$this->updateMenuItems($model);
	
		if (isset($_GET['ItemCompanyCategory']))
		{
			$model->setAttributes($_GET['ItemCompanyCategory']);
	...
	{
		$model = new ItemCompanyCategory('search');
		if( !($model->checkPermission ('itemCompanyCategory/admin')))	throw new CHttpException(403, Yii::t('app','You are not allowed to access this page.'));
		$model->unsetAttributes();
		$this->updateMenuItems($model);
