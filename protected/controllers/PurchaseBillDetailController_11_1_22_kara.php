...
class PurchaseBillDetailController extends GxController {
	...
	/* public function actionAjaxPBillTax() {
		...
		if (isset ( $_POST ['item_detail_id'] ) && isset ( $_POST ['vendor_id'] )) {
			...
			$taxes = Tax::model ()->findAll ();
			$option .= '<select class="form-control" id="PO_item_detaill_list_id" name="PurchaseBillDetail[tax_id]"><option value="" id="ckbCheckAll">-Select-</option>';
			if ($taxes) {
				foreach ( $taxes as $taxx ) {
					$selected = '';
					if ($tax != null) {
	...
	public function actionAjaxItems() {
		...
		if (isset ( $_POST ['item_id'] )) {
			...
			/* $option .= '<select class="form-control" onChange="checkTaxes()" name="PurchaseBillDetail[item_detail_id]" id="PurchaseBillDetail_item_detail_id"><option value="" id="ckbCheckAll">-Select-</option>';
			if ($itemdetails) {
		...
		if ($bill) {
			if (isset ( $_POST ['credit_note'] ) && isset ( $_POST ['bill_amount'] )) {
				...
				/* 	$criteria1 = new CDbCriteria ();
					if($bill->original_vendor_id == null){
						$criteria1->compare ( 'vendor_id', $bill->vendor_id );
					}
					$criteria1->compare ( 'outlet_id', $bill->outlet_id );
					$criteria1->compare ( 'credit_note_id', $model->id );
					$return = ItemReturn::model ()->find ( $criteria1 );
					if($return == null){
						$criteria1 = new CDbCriteria ();
						$criteria1->compare ( 'outlet_id', $bill->outlet_id );
						$criteria1->compare ( 'credit_note_id', $model->id );
						$returnitem = ItemReturn::model ()->find ( $criteria1 );
						if(in_array($return, explode(',',$bill->original_vendor_id))){
							$return = $returnitem;
						}
					} */
					/* if ($return) { */
...
// 					}
	 ...
	 * $dataProvider = new CActiveDataProvider ( 'PurchaseBillDetail' );
...
// 					) );
// 					if ($advancePayment) {
// 						$advancePayment->balance_amt = ($advancePayment->balance_amt) - $purchasebill->net_bill_amount;
// 						if ($advancePayment->save ()) {
// 							$advancelog = new AdvanceLogs ();
// 							$advancelog->amount = $purchasebill->net_bill_amount;
// 							$advancelog->advance_payment_id = $advancePayment->id;
// 							$advancelog->type_id = AdvanceLogs::TYPE_SUBSTRACT;
// 							$advancelog->save ();
// 						}
// 					}
