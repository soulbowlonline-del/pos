...
class PermissionController extends GxController {
	...
	{
		$model = $this->loadModel($id, 'Permission');
	...
	public function actionDelete($id) 
	{
		...
		if (Yii::app()->getRequest()->getIsPostRequest()) {
			$this->loadModel($id, 'Permission')->delete();
	...
	public function actionSearch()
	{
		...
		{
			$model->setAttributes($_GET['Permission']);
	...
	{
		$model = new Permission('search');
		if( !($model->checkPermission ('permission/admin')))	throw new CHttpException(403, Yii::t('app','You are not allowed to access this page.'));
		$model->unsetAttributes();
		$this->updateMenuItems($model);
