...
class MrnDetailController extends GxController {
	...
	public function accessRules() {
		return array(
				...
				array('allow', 
					'actions'=>array('create','update', 'search','admin','delete','ajaxupdate','ajaxItems','ajaxTax','ajaxCreate','ajaxMrnNo','sendEmail'),
					'users'=>array('@'),
					),
				/*array('allow', 
	...
	public function actionSendEmail($id)
    {
        ...
        $mPDF1->WriteHTML($this->renderPartial('/mrn/_pdf', array(
            'mrn' => $mrn,
            'poid' => $id
        ), true));
        ...
        if ($vendor) {
               ...
               $to="cs@soulbowl.in";
                ...
                $view = $this->renderPartial('/mail/mrn_report', array(
                    'pomodel' => $mrn
                ), true);
	...
	{
		$model = $this->loadModel($id, 'MrnDetail');
	...
	public function actionDelete($id) 
	{
		...
		if (Yii::app()->getRequest()->getIsPostRequest()) {
			$this->loadModel($id, 'MrnDetail')->delete();
	...
	public function actionIndex($id = null,$mrnid= null)
	{
		...
		if( !($model->checkPermission ('mrnDetail/index')))	throw new CHttpException(403, Yii::t('app','You are not allowed to access this page.'));
		...
		if (isset($_GET['MrnDetail']))
			$model->setAttributes($_GET['MrnDetail']);
	...
	public function actionAdmin($id = null,$mrnid= null) 
	{
		...
		$model = new MrnDetail('search');
		if( !($model->checkPermission ('mrnDetail/admin')))	throw new CHttpException(403, Yii::t('app','You are not allowed to access this page.'));
	...
	{	
		...
		switch( $this->action->id)
		{
			...
			case 'admin':
				...
				//	$this->menu[] = array('label'=>Yii::t('app', 'List'), 'url'=>array('index'),'icon'=>'icon-th-list icon-white');
					$this->menu[] = array('label'=>Yii::t('app', 'Add Item'), 'url'=>array('create' ,'id'=>$mrnid),'icon'=>'icon-plus icon-white');
				}
				break;				
			default:
			case 'view':
...
//print_r($_POST);exit;		   
		...
		$pomodel = PurchaseOrder::model()->findByAttributes(array('mrn_id'=>$id,'vendor_id'=>$mrn->vendor_id));
		$updated = true;
		if($pomodel == null){
			$pomodel = new PurchaseOrder();
			$updated = false;
			...
			$pomodel->gross_amt = $_POST ['gross_amt'];
				...
				if($pomodel->save()){
					...
					$email = '' ;
					$vendor = Vendor::model()->findByPk($pomodel->vendor_id);
					if($vendor){
						$to_id = $vendor->create_user_id;
					...
					$view = $this->renderPartial ( '/mail/purchase_order', array (
							'pomodel'=>$pomodel
					), true );
				...
				$model=$this->loadModel($key,'MrnDetail');
				$item = Item::model()->findByPk($model->item_id);
					$mrnmodel = $this->loadModel($model->mrn_id,'Mrn');
 ...
 */					
						...
						if(isset( $mrnIdAll['maxData']) && isset( $mrnIdAll['minData']))
					{
						...
						if($qty != '' && $qty != '0'){
						$podetailmodel = PurchaseOrderDetail::model()->findByAttributes(array('purchase_order_id'=>$pomodel->id,'outlet_id'=>$pomodel->outlet_id,'item_id'=>$model->item_id,'item_detail_id'=>$model->item_detail_id));
						if($podetailmodel == null){
							$podetailmodel = new PurchaseOrderDetail();
						}
        	...
        	if(($mrn) && ($mrn->status == MrnDetail::STATUS_DONE)){
        		...
        		$mrndetailmodels = MrnDetail::model()->findAllByAttributes(array('mrn_id'=>$id));
        		if($mrndetailmodels){
