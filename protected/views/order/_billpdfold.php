...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
  <tbody>
   ...
   <tr>
    ...
    <?php $customer = Customer::model()->findByPk($order->customer_id);
	$user = User::model()->findByPk($order->create_user_id);?>
    <tr>
    ...
    <tr>
     ...
     <tr>
     <td colspan="4" style="text-align:left; font-size:14px; font-weight:bold">Date: <?php echo date('Y-m-d h:i a',strtotime($user->create_time));?></td>
    </tr>
  ...
  <tbody>
	...
	if($items){
		foreach($items as $item){
			$product = Item::model()->findByPk($item->item_id);
			if($product){
			?>
    <tr>
  ...
  <tbody>
	...
	if($itemms){
		foreach($itemms as $item){
			$criteria = new CDbCriteria ();
			$criteria->addCondition ( 'order_id =' . $order->id );
			$criteria->addCondition ( 'tax_id =' . $item->tax_id );
