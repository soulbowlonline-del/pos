...
<section class="content-header">
  ...
  <a href="<?php echo Yii::app()->createUrl('itemReturn/list');?>"
		class="btn btn-info export-btn" target="_blank">Merge</a>
	<a href="<?php echo Yii::app()->createUrl('itemReturnItem/printPdf',array('vendor_id'=>$vendor_id, 'outlet_id' => $outlet_id));?>" class="btn btn-primary export-btn" target="_blank">Print Pdf</a>
...
?>
		...
		<?php echo $form->dropDownListRow($model, 'vendor_id', GxHtml::listDataEx(Vendor::model()->findAllByAttributes(array('status'=>Vendor::STATUS_ACTIVE), ["order" => "name asc"])), array('prompt' => Yii::t('app', 'Select Vendor'))); ?>
...
$form = $this->beginWidget ( 'bootstrap.widgets.TbActiveForm', array (
		...
		'type' => 'horizontal',
		// 'action'=> Yii::app()->createUrl('ItemReturnItem/create'),
	...
	<div class="add-item">
								<div class="col-md-10 col-sm-12 col-xs-12 margin10">
									...
									<div class="row">
											...
											<label class="control-label" for="">Bar Code</label>
                  <?php echo $form->textField($model,'bar_code',array('class'=>'form-control','id'=>'ItemReturnItem_bar_code')); ?>
										</div>
...
<input type="hidden" name="ItemReturnItem[item_id]" id="item_hsn_code">
   ...
   <?php echo $form->error($model,'item_id');?>
                  ...
                  <?php  echo $form->textField($model,'price',  array('class'=>'form-control','id'=>'ItemReturnItem_price_field')); ?>
                </div>
    ...
    'columns'=>array(
		...
		/*	 array(
    		...
    		array(
    				...
    				'header'=>'qty',
    				'value'=>'GxHtml::activeTextField($data,\'qty\', array("id"=>"req_input$data->id","class"=>"req_input"))',
    				'type'=>'raw',
			...
			array (
    				'header' => 'Sent In Api',
    				'value' => 'GxHtml::dropDownList("ItemReturnItem[type_id]",$data->type_id,$data->getAPIOptions(),array("id"=>"api_type$data->id","class"=>"api_type"))',
    				'type' => 'raw',
    				'htmlOptions' => array (
    						'class' => 'api_type'
    				)
    		...
    		array(
    				...
    				// 'value'=>'GxHtml::valueEx($data->tax)',
						'value' => 'GxHtml::dropDownList("PurchaseBillDetail[tax]",$data->tax_id,$data->getAllTaxOptions($data->tax_id, $data->vendor_id, $data->outlet_id),array("id"=>"select_tax$data->id","class"=>"select_tax"))',
						'type' => 'raw',
    		...
    		array(
    				...
    				'header'=>'Amt',
    				'value'=>'GxHtml::activeTextField($data,\'total_amt\',array("id"=>"total_amount_input$data->id","class"=>"total_amount_input"))',
    				'type'=>'raw',
...
$(document).ready(function(){
	...
	$('#ItemReturngrn_no').change(function(){
		...
		$.ajax({
	       url: '<?php echo Yii::app()->createUrl('ItemReturnItem/ajaxbillno'); ?>',
	       type: 'post',
	...
	$("#approve").click(function (event) 
	{
		...
		if(credit_note_no != '' && invoice_no != '' && bill_no != '' && grn_no != '' && credit_note_date != ''){
			$.ajax({
 		       url: '<?php echo Yii::app()->createUrl('ItemReturnItem/ajaxCreditBillNo'); ?>',
 		       type: 'post',
 		        data: {
			...
			$("td.api_type select").each(function(key,value){
				...
				$.ajax({
							url: '<?php echo Yii::app()->createUrl('ItemReturnItem/ajaxupdate'); ?>',
							type: 'post',
	...
	$("#update").click(function (event) 
	{
		...
		if(confirm("Do you really want to update?") == true){
			...
			$("td.api_type select").each(function(key,value){
				...
				$.ajax({
							url: '<?php echo Yii::app()->createUrl('ItemReturnItem/ajaxupdateonly'); ?>',
							type: 'post',
...
function checkTaxTable(tax_id,id){
  ...
  //  console.log('igst'+igst);
	...
	jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo CController::createUrl('ItemReturnItem/ajaxTaxTable') ?>',
...
function checkTaxes(){
	 ...
	 if(vendor_id != ''){
	    jQuery.ajax({
	       'type': 'POST',
	       'url': '<?php echo CController::createUrl('ItemReturnItem/ajaxTax') ?>',
...
$('#ItemReturnItem_mrs_id').change(function(){
	...
	var mrsid = $('#ItemReturnItem_mrs_id').val();
	var url = '<?php echo Yii::app()->createUrl('ItemReturnItem/admin')?>/id/'+vendor_id+'/mrsid/'+mrsid;
...
console.log('req_qty'+req_qty+'vendor_id'+vendor_id+'outlet_id'+outlet_id);
 ...
 jQuery.ajax({
     'type': 'POST',
     'url': '<?php echo CController::createUrl('ItemReturnItem/create') ?>',
