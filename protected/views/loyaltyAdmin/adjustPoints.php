<?php
$this->pageTitle = 'Adjust Customer Points';
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <select name="customer_id" id="customer-select" class="form-control select2" required style="width: 100%;">
                                    ...
                                    <?php 
                                    $customers = Customer::model()->findAll(array('order' => 'name ASC'));
                                    foreach ($customers as $customer): 
                                        $isSelected = isset($selectedCustomerId) && $selectedCustomerId == $customer->id;
                                    ?>
                                        <option value="<?php echo $customer->id; ?>" 
                                                data-phone="<?php echo CHtml::encode($customer->contact_no); ?>" 
                                                data-email="<?php echo CHtml::encode($customer->email); ?>"
                                                <?php echo $isSelected ? 'selected="selected"' : ''; ?>>
                                            <?php echo CHtml::encode($customer->name); ?>
                                            <?php if ($customer->contact_no): ?>
                                                (<?php echo CHtml::encode($customer->contact_no); ?>)
                                            <?php endif; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <div class="box-footer">
                        <div class="col-sm-offset-3 col-sm-9">
                            ...
                            <?php if (isset($selectedCustomerId) && $selectedCustomerId): ?>
                                <a href="<?php echo $this->createUrl('viewTransactions', array('id' => $selectedCustomerId)); ?>" class="btn btn-default">
                                    <i class="fa fa-arrow-left"></i> Back to Transactions
                                </a>
                            <?php else: ?>
                                <a href="<?php echo $this->createUrl('customers'); ?>" class="btn btn-default">
...
$(document).ready(function() {
    ...
    function loadCustomerLoyaltyInfo(customerId) {
        $.ajax({
            url: '<?php echo $this->createUrl("getCustomerLoyaltyInfo"); ?>',
            method: 'POST',
