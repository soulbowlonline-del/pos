...
public static function rrmdir($dir) {
	...
	{
		if ( $title == null )$title = (string)$this;
		$image_link = CHtml::image($this->getUrl('thumbnail'),$title,array('width'=>'120','height'=>'90'));
	...
	{
		return array(
				'my' => array('condition' => 'user_id =' . Yii::app()->user->id ),//. ' OR to_list= '. Yii::app()->user->model->username),
				'latest' => array('order'=> 'create_time DESC '),
				'active' => array('condition'=> 'state_id = 1'),
		);
	}
	...
	public function defaultScope()
	{
		...
		else if($this->hasAttribute('create_time'))	return array( 'order'=> 't.create_time DESC ');
	...
	protected function beforeDelete()
	{
		...
		{
			Comment::model()->deleteAllByAttributes(array ('model_type'=> get_class($this), 'model_id'=>$this->id));
		} */
	...
	public function formatDateTime1($attribute, $bShowTime=false)
	{
		$format  = "F jS, Y " ;
		if ( $bShowTime ) $format  = "F jS, h:m a" ;
	...
	public function getImage($image)
	{
		...
		if($image)
		{
			return CHtml::image(Yii::app()->createAbsoluteUrl('user/download',array('file'=>$image)),"",array('height'=>100));
			
		}
		else {
			$default_img = 'default.png';
					
					return CHtml::image(Yii::app()->createAbsoluteUrl('user/download',array('file'=>$default_img)));
		}
	}
	...
	public function checkPermission($url){
		// Per-request memoization. The admin layout calls this ~95x per page; the
		// original ran 2 uncached queries + hydrated the full role-permission set
		// every call (~190 queries/page). Here we load all permissions and the
		// role's permission ids once, then answer from memory. Same result.
		static $permIdByUrl = null;   // url => permission id
		static $rolePermSet = array(); // role_id => array(permission_id => true)
		...
		if ($permIdByUrl === null) {
			$permIdByUrl = array();
			foreach (Permission::model()->findAll() as $p) {
				$permIdByUrl[$p->url] = $p->id;
			}
		}
		if (!isset($rolePermSet[$roleId])) {
			$set = array();
			foreach (RolePermission::model()->findAllByAttributes(array('role_id' => $roleId)) as $rp) {
				$set[$rp->permission_id] = true;
			}
			$rolePermSet[$roleId] = $set;
		}
	...
	public function getItemOptions($vendor_id = null){
		...
		if($vendor_id != null){
			$itemvendor_ids = array();
			$vendor = Vendor::model()->findByAttributes(array('id'=>$vendor_id));
			if($vendor){
				$itemvendors = ItemVendor::model()->findAllByAttributes(array('vendor_id'=>$vendor->id));
				if($itemvendors){
	...
	public function getItemOptionbarcodes(){
		$item_ids = $this->getItemOptionIds();
		$list = array();
		$criteria = new CDbCriteria ();
	...
	public function getItemOptionIdsInBarcode($match_item_id ,$match_mrp,$match_hsn_code,$match_product_code,$match_purchase_price,$match_company_id,$is_vendor){
		$list = array();
		$criteria = new CDbCriteria ();
		if($match_item_id != null)
		{
			...
			$criteria->params = array (
					':title' => trim (  $match_item_id ) . '%'
			);
		
			//$criteria->compare('title',$match_item_id,true);
		}
		...
		{
			$criteria->compare('company_id',$match_company_id,true);
		}
		
		if($match_mrp != null)
		{
			$criteria->addCondition('mrp ="'. $match_mrp.'"');
		}
		if($match_hsn_code != null)
		{
			$criteria->addCondition('hsn_code = "'. $match_hsn_code.'"');
			
		}
		if($match_product_code != null)
		{
			$criteria->addCondition('item_code ="'. $match_product_code.'"');
			
		}
		
		
	//	Yii::log ( CVarDumper::dumpAsString ( $match_purchase_price ), CLogger::LEVEL_WARNING, '$match_purchase_price' );
		
		if($match_purchase_price != null)
		{
			//$criteria->addCondition('purchase_price = "'. $match_purchase_price.' "');
			$criteria->compare ( 'purchase_price', $match_purchase_price );
		}
	...
	public function customermailsend($to,$from,$subject,$message,$attachment,$type,$name){
		...
		if($attachment != ''){
		$mail->AddAttachment(dirname(__FILE__).'/../../wdir/uploads/'.$attachment, $name = $name,  $encoding = 'base64', $type = $type);
		...
		//$mail->AddAttachment(wdir/uploads/pdf,'filename.pdf');
		Yii::log ( CVarDumper::dumpAsString ( $mail ), CLogger::LEVEL_WARNING, '$mail' );
		Yii::log ( CVarDumper::dumpAsString ( $to ), CLogger::LEVEL_WARNING, '$to' );
		Yii::log ( CVarDumper::dumpAsString ( $from ), CLogger::LEVEL_WARNING, '$from' );
		...
		{
			Yii::log ( CVarDumper::dumpAsString ( $e ), CLogger::LEVEL_WARNING, '$mail_error' );
			return 2;
		}
	...
	public function mailsend($to,$from,$subject,$message){
		...
		$mail->AddAttachment(dirname(__FILE__).'/../../wdir/uploads/pdf/filename.pdf', $name = 'po.pdf',  $encoding = 'base64', $type = 'application/pdf');
	...
	public function getVendorDataOptions(){
		$list = array();
		$vendors = Vendor::model()->findAllByAttributes(array('status'=>Vendor::STATUS_ACTIVE));
		if($vendors){
	...
	public function getCompanyBarcode($id){
		...
		$itemDetail = ItemDetail::model()->findByPk($id);
		if($itemDetail){
			if($itemDetail->company_bar_code == ItemDetail::IS_COMPANY){
				return "readOnly";
			}else{
				return "";
			}
	...
	public function getSessionStartDate(){
		$start_date = '';
		if(Yii::app()->session['select_session_id'] != ''){
			$session_id = Yii::app()->session['select_session_id'];
			$session = Session::model()->findByPk($session_id);
			if($session){
				$getSessions = explode('-',$session->name);
				if(isset($getSessions['0'])){
					$start_date = $getSessions['0'].'-04-01';
				}
			}
		}
		return $start_date;
	...
	public function getSessionEndDate(){
		...
		if(Yii::app()->session['select_session_id'] != ''){
			...
			if($session){
				$getSessions = explode('-',$session->name);
				Yii::log ( CVarDumper::dumpAsString ( $getSessions ), CLogger::LEVEL_WARNING, '$getSessions' );
				if(isset($getSessions['1'])){
					$end_date = $getSessions['1'].'-03-31';
				}
			}
		}
		return $end_date;
	...
	public function isAllowCreate(){
		...
		if(Yii::app()->session['select_session_id'] != ''){
			...
			if($session){
				$getSessions = explode('-',$session->name);
				if(isset($getSessions['1']) && isset($getSessions['0'])){
					if(($getSessions['0'] == $year) && ($getSessions['1'] == $yearadd)){
						return true;
					}
				}
			}
	...
	public function getItemCustomerName(){
		$customer = Customer::model()->findByPk($this->order->customer_id);
		if($customer){
