...
public function actionDeleteAssets()
	...
	public function AddPanel($title, $objects, $relations, $view,$partialview="_list",$model = null, $addMenu = true)
	{
		...
		if ( $objects )
		{
			...
			{
				$content = $this->renderPartial('/'.$view.'/'.$partialview,array('dataProvider'=>$dataProvider),true);
				$this->tabs_data[] = array('label'=>$title, 'content'=>$content, 'active'=> count ( $this->tabs_data) == 0 ? true: false);
			}
		}
	}
	public function AddNewPanel($title, $objects, $relations, $view,$partialview="_list",$model = null, $addMenu = true)
	{
		if ( $addMenu ) $this->menu[] = array('label'=>Yii::t('app', 'Add ') . $title , 'url'=>array($view .'/create', 'id'=> $model ? $model->id :null, 'icon'=>'icon-plus icon-white'));
		...
		if ( $objects )
		{
	        ...
	        $criteria->addCondition('customer_id ='.$model->id);
	...
	public function downloadAs($title, $tpl, $name, $content, $type='pdf')
	{
		...
		if( $type == 'pdf' )
		{
			...
			$pdf->SetXY(100, 180);
			$pdf->write2DBarcode('www.toxsl.com', 'QRCODE,H', '', '', 50, 50, $style, 'N');
	...
	public function actionDownload($file = null, $thumbnail = false)
	{
		...
		public function actionDownload1($file_name)
	{

		$file = Yii::app()->basePath.'/../wdir/upload_bar_img/'.$file_name;
		if(file_exists($file))
		{
			//echo 'scd'; exit;
			return Yii::app()->getRequest()->sendFile($file_name, @file_get_contents($file));
		}
	}
	...
	public function actionReject($id,$model)
	{
		$reject = $this->loadModel($id, $model);
		if($reject)
		{
			$model = new Message;
			$model->to_id = $reject->createUser->id;

			$this->performAjaxValidation($model, 'message-form');

			if (isset($_POST['Message'])) {
				$model->setAttributes($_POST['Message']);
				$model->state_id = Message::STATUS_NEW;

				if ($model->save())
				{
					$reject->state_id = Job::STATUS_DRAFT;
					$reject->saveAttributes(array('state_id'));
					$model->addRejectInPending();
